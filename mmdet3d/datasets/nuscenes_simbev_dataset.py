...
class NuScenesSimBEVDataset(Custom3DDataset):
    r"""NuScenes Dataset.
    ...
    def sample_augmentation(self):
        fH, fW = self.data_aug_conf['final_dim'] # [448, 800] 
        if self.use_valid_flag:
            # validation/test
            # do a perfect resize
            resize_dims = (fW, fH)
    ...
    def get_data_info(self, index: int) -> Dict[str, Any]:
        ...
        if self.modality["use_camera"]:
            ...
            for _, camera_info in info["cams"].items():
                ...
                # simple bev
                intrin = camera_info['camera_intrinsics']
                data["intrins"].append(intrin)
                rot = Quaternion(camera_info['sensor2ego_rotation']).rotation_matrix
                data["rots"].append(rot)
                tran = camera_info['sensor2ego_translation']
                data["trans"].append(tran) #

        annos = self.get_ann_info(index)
        data["ann_info"] = annos
        data["trans"] = np.stack(data["trans"]) # prevent pad for DataContainer
        return data
            ...
            for i, box in enumerate(boxes):
                ...
                if np.sqrt(box.velocity[0] ** 2 + box.velocity[1] ** 2) > 0.2:
                    if name in [
                        ...
                        attr = NuScenesSimBEVDataset.DefaultAttribute[name]
                else:
