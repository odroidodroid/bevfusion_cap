...
def count_ops_and_params(model, example_inputs, layer_wise=False):
    ...
    ori_model = model 
    model_cp = copy.deepcopy(model.to('cpu')) # deepcopy to avoid changing the original model
    model_cp.to('cuda')
    flops_model = add_flops_counting_methods(model_cp)
    flops_model.eval()
    ...
    del model, flops_model, ori_model
